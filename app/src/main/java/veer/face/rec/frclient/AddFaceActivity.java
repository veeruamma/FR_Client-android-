...
public class AddFaceActivity extends AppCompatActivity {
    ...
    private static String TAG= "Add Face Activity";
...
//    private Mat tempMat ;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        showImage = (ImageView) findViewById(R.id.showFace);
        openCamera = (ImageView) findViewById(R.id.openCamera);
        processButton = (Button) findViewById(R.id.addFaceButton);
        ...
        processButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                final String faceName = name.getText().toString().trim();
                if(encodedFaceMat != null && faceName != null){
                    OkHttpClient okHttpClient = new OkHttpClient();

                    RequestBody requestBody = new FormBody.Builder()
                            .add("encodedFace", encodedFaceMat)
                            .add("faceName", faceName).build();

//                    Request request = new Request.Builder().url("http://192.168.56.241:5000/train_features")
//                            .post(requestBody).build();
                    Request request = new Request.Builder().url("https://veer-fr-webservice.herokuapp.com/train_features")
                            .post(requestBody).build();

                    okHttpClient.newCall(request).enqueue(new Callback() {
                        @Override
                        public void onFailure(@NotNull Call call, @NotNull IOException e) {
                            Toast.makeText(AddFaceActivity.this, e.getMessage(), Toast.LENGTH_LONG).show();
                        }

                        @Override
                        public void onResponse(@NotNull Call call, @NotNull Response response) throws IOException {
                            final TextView textView = findViewById(R.id.trainRes);
